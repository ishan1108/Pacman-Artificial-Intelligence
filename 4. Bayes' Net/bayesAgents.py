...
def fillObsCPT(bayesNet, gameState):
    ...
    values = [BOTTOM_LEFT_VAL, TOP_LEFT_VAL, BOTTOM_RIGHT_VAL, TOP_RIGHT_VAL]
    counter = 0
    for position in houses:
        houseWalls = gameState.getHouseWalls(position)   
        for houseWall in houseWalls:
            houseWall = OBS_VAR_TEMPLATE % houseWall
            factor = bn.Factor([houseWall], HOUSE_VARS, bayesNet.variableDomainsDict())
            possibleAssignment = factor.getAllPossibleAssignmentDicts()         
            for possibleAssignments in possibleAssignment:
                probabilityRed = 0
                probabilityBlue = 1 - probabilityRed
                if possibleAssignments[FOOD_HOUSE_VAR] == values[counter]:
                    if possibleAssignments[houseWall] == RED_OBS_VAL:
                        probabilityRed = PROB_FOOD_RED
                        probabilityBlue = 1 - probabilityRed
                    else:
                        if possibleAssignments[houseWall] == BLUE_OBS_VAL:
                            probabilityRed = 1 - PROB_FOOD_RED
                            probabilityBlue = PROB_FOOD_RED
                elif possibleAssignments[GHOST_HOUSE_VAR] == values[counter]:
                    if possibleAssignments[houseWall] == RED_OBS_VAL:
                        probabilityRed = PROB_GHOST_RED
                        probabilityBlue = 1 - PROB_GHOST_RED
                    else:
                        if possibleAssignments[houseWall] == BLUE_OBS_VAL:
                            probabilityRed = 1 - PROB_GHOST_RED
                            probabilityBlue = PROB_GHOST_RED
                else:
                    if possibleAssignments[houseWall] == NO_OBS_VAL:
                        probabilityRed = 1
                        probabilityBlue = 0
                factor.setProbability(possibleAssignments, probabilityRed)
            bayesNet.setCPT(houseWall, factor)
        counter = (counter + 1) % 4
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    best = 0
    possibleAssignment = factor.getAllPossibleAssignmentDicts()
    for assignment in possibleAssignment:
        if factor.getProbability(assignment) > max:
            max = factor.getProbability(assignment)
            best = assignment
    return best
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        right = 0
        possibleAssignment = factor.getAllPossibleAssignmentDicts()
        for assign in possibleAssignment:
            if assign[FOOD_HOUSE_VAR] == TOP_LEFT_VAL:
                if assign[GHOST_HOUSE_VAR] == TOP_RIGHT_VAL:
                    left = factor.getProbability(assign)
            else:
                if assign[FOOD_HOUSE_VAR] == TOP_RIGHT_VAL:
                    if assign[GHOST_HOUSE_VAR] == TOP_LEFT_VAL:
                        right = factor.getProbability(assign)
        
        left_reward = left * WON_GAME_REWARD 
        left_ghost = (1  - left) * GHOST_COLLISION_REWARD
        leftExpectedValue = left_reward + left_ghost
        right_reward = right * WON_GAME_REWARD
        right_ghost = (1 - right) * GHOST_COLLISION_REWARD
        rightExpectedValue = right_reward + right_ghost
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        "*** YOUR CODE HERE ***"
        explore = self.getExplorationProbsAndOutcomes(evidence)
        for x in explore:
            evidence.update(x[1])
            values = self.computeEnterValues(evidence, enterEliminationOrder)
            expectedValue = expectedValue + x[0] * max(values)
