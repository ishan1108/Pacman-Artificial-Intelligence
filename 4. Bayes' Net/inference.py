...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        "*** YOUR CODE HERE ***"
        allCPT = bayesNet.getAllCPTsWithEvidence(evidenceDict)
        for x in eliminationOrder:

            allCPT, new = joinFactorsByVariable(allCPT, x)
            
            length = len(new.unconditionedVariables())
            if length != 1:
                elim = eliminate(new, x)
                allCPT.append(elim)
        joined = joinFactors(allCPT)
